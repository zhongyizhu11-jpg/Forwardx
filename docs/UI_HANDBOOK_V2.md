...
这条被推翻过两次，所以要把来龙去脉写清楚。

第一版每层都有自己的一点灰（页面 #f4f4f6、分组 #f7f7f9、输入框填充 #f1f1f4），
合起来一屏四种灰，而且全都偏蓝 —— 读起来「发闷」。第二版反过来「灰底托白卡、卡片
不描边」：页面浅灰、卡片纯白，靠底色差成形，像 iOS 的设置页。它成立，但气质是
iOS，不是参考站那种「一张白纸上的干净表格」。

2026-09 这一版照参考站（New API / Vexo）来：**页面和卡片都是纯白，卡片靠一圈 1px 的
弱线（`--fx-stroke-weak`，约 10% 的黑）成形，不投影。** 深色不是黑，是一层炭灰
（页面 `#1e1e1e`、卡片 `#2a2a2a`），线是 10% 的白。

规矩只有一条：**线只画一次。** 卡片描边了，卡里就不再套框 —— 第一版三层嵌套圆角
矩形的病根就是每一层都在画框。卡内分组用间距和分隔线（`border-t`），确实要再分一格
时用 `--fx-l2-group`（比白纸深一点点的灰），不再描边。

中性色一律 **R=G=B**（偏蓝的灰是第一版的真正病根）。
悬停/按下用 `--fx-hover`，它是**瞬时**反馈，不是静态的面。

### 除了这一圈线，谁来成形

- **选中态**：强调色。分段控件的选中项是白块 + 1px 软影（手机上是天蓝实底），侧栏选中项是
  淡天蓝底 + 天蓝字 —— 一屏扫过去，有颜色的那块就是「你在这里」。
- **静态标记**（TCP / 计数徽标）：描边成形，底色留白。
- **输入框**：基础线那一档描边（比卡片的弱线深一点）；直接坐在页面上的搜索框、筛选按钮
  用弱线，和卡片同一种线。
...
| 令牌 | Tailwind | 用在哪 |
|---|---|---|
| `--fx-elevation-flat` | — | 普通卡片：靠一圈弱线成形，不投影 |
...
| 分类 | `--fx-cat-indigo` / `-teal` / `-orange` / `-pink` | `text-[var(--fx-cat-indigo)]` | 「它是哪一种」：转发工具标签、图表第 6 位以后。靛而不是蓝，蓝和天蓝的强调色撞了 |
...
ForwardX 的识别色是天蓝（字和按钮 `#0b74d1`，线、点、图表 `#3ea4ec`，淡底是天蓝兑 14%），它就是这个强调色；路径色跟着它走。上一版的薰衣草紫留在个性化预设「薰衣草」里。
...
（DataSectionError）。都是一块 surface，空状态描一圈弱线（坐在卡里时不描）；搜着东西没搜到时说「未找到匹配…」，不说「暂无…」。
