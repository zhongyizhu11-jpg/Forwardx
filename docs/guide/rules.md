...
资源类型可以在编辑时切换。跨类型修改只保留名称、入口端口、协议、目标地址、目标端口等基础字段；原资源专属的主备线路、PROXY Protocol 或其他参数不会带到不支持它们的新资源。
...
主备线路（一条主线路加几条备用线路，主线路出问题时自动换）直接在规则表单里勾选，怎么用见 [主备线路](./failover.md)。
