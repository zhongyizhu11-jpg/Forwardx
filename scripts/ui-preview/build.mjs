...
const darkLogo=await fs.readFile(path.join(root,'client/public/logo-dark.png'));
const doc=`<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><style>${css}</style></head><body><div id="root"></div><script>window.__PREVIEW_LOGO__="data:image/png;base64,${logo.toString('base64')}";window.__PREVIEW_LOGO_DARK__="data:image/png;base64,${darkLogo.toString('base64')}";</script><script>${js.replaceAll('</script','<\\/script')}</script></body></html>`;
