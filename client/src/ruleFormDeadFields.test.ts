...
test("表单字段数守住上限，别再长回去", () => {
  /*
    这是个棘轮，不是硬性设计约束。清理之后是 17，现在 18。

    涨的那一个是 failoverProbeTarget（主出站的健康探测目标）。它值得涨：不填这个
    的话，中转用 gost/realm 这类用户态转发时，健康检查只能探到中转本身 —— 中转的
    上游断了主备不会切，而面板上一切正常。它也只出现在「更多设置 → 出站策略」里，
    而出站策略本身默认是关的，第一屏看不到它。
  ...
  */
  ...
  assert.ok(
    fields.length <= 18,
    `创建转发的表单涨到了 ${fields.length} 个字段（上限 18）。`
